...
RETURNS integer AS $$
    ...
    FROM (
        SELECT
            string_to_array(left_portions[1], '.')::bigint[],
            string_to_array(right_portions[1], '.')::bigint[],

            (SELECT array_agg(CASE WHEN v ~ '^\d+$' THEN ROW(''::text, v::bigint) ELSE ROW(v) END) FROM unnest(string_to_array(lower(left_portions[2]), '.')) x (v)),
            (SELECT array_agg(CASE WHEN v ~ '^\d+$' THEN ROW(''::text, v::bigint) ELSE ROW(v) END) FROM unnest(string_to_array(lower(right_portions[2]), '.')) x (v))
        FROM (VALUES (
